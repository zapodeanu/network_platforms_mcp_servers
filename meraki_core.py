...
# Normalize root logger records to meraki_core in shared log file.
class _MerakiCoreLogNameFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        if record.name == "root":
            record.name = "meraki_core"
        return True


_meraki_log_handler = logging.FileHandler(os.path.join(PATH, "logs", "meraki_mcp.log"), mode="a")
_meraki_log_handler.addFilter(_MerakiCoreLogNameFilter())

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - PID:%(process)d - %(name)s - %(levelname)s - %(message)s",
    handlers=[_meraki_log_handler],
    force=True
)

logging.info("=" * 80)
logging.info("Meraki MCP server with API explorer started")
...
class MerakiOrganizationManager:
    ...
    def _load_organizations(self):
        ...
        try:
            with open(self.organizations_config_path, 'r', encoding='utf-8') as f:
...
class EnhancedMultiOrgMerakiServer:
    def __init__(
            self,
            server_instance_name: str = "enhanced-multi-org-meraki",
            stdio_server_name: str = "enhanced-declarative-meraki",
    ):
        logging.info("Initializing Meraki MCP server...")

        self.server_instance_name = server_instance_name
        self.stdio_server_name = stdio_server_name
        self.server = Server(self.server_instance_name)
        ...
        self.analytics_file = f"{PATH}/logs/meraki_mcp_explorer_analytics.json"
        ...
        self._setup()
        logging.info("Meraki MCP server ready!")
    ...
    def _setup(self):
        ...
        logging.info("Setting up Meraki MCP server handlers...")
                ...
                Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            ...
                            "method": {
                                "type": "string",
                                "description": "Optional HTTP method filter (GET only for safety)",
                                "enum": ["GET"]
                            },
                            "limit": {
                                "type": "integer",
                ...
                Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            ...
                            "method": {
                                "type": "string",
                                "description": "HTTP method (GET only for safety)",
                                "enum": ["GET"],
                                "default": "GET"
                            },
                            "organization": {
        ...
        logging.info("Meraki MCP server handlers registered")
        ...
        if method != "GET":
            return {
                'content': [{'type': 'text', 'text': json.dumps({
                    ...
                    "allowed_methods": ["GET"],
                    "message": "Write operations (POST/PUT/DELETE/PATCH) are disabled for safety"
                }, indent=2)}]
            }
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                if method != "GET":
                    raise Exception(
                        ...
                        "Write operations (POST/PUT/DELETE/PATCH) are disabled for safety"
                    )
